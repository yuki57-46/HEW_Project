...
void Model::MakeMesh(const void* ptr, float scale, Flip flip)
{
	...
	{
		// 頂点の作成
		m_meshes[i].vertices.resize(pScene->mMeshes[i]->mNumVertices);	// 頂点の必要数分確保
		for (unsigned int j = 0; j < m_meshes[i].vertices.size(); ++j)
		{
			...
			// UV(一つのメッシュに複数のテクスチャが貼り付けられるため、テクスチャの枚数に応じてUVが設定されている
		...
		// インデックスの作成
		// mNumFacesはポリゴンの数を表す(1ポリゴンで3インデックス
		m_meshes[i].indices.resize(pScene->mMeshes[i]->mNumFaces * 3); // インデックスの必要分確保
